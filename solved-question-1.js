...
const getEldest = (data) => {
    let FetchedDataFromAray = Math.max.apply(
        Math, data.map(function(o){
            return o.age || o.leeftijd;
        }));
        let ArrayOjectDataFiltering = data.filter(function(item){
            return item.age === FetchedDataFromAray || item.leeftijd === FetchedDataFromAray;
        })[0];
        return  ArrayOjectDataFiltering.naam || ArrayOjectDataFiltering.name;
